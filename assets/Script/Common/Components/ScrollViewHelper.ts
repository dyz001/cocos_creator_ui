...
export class ScrollViewHelper extends cc.Component {
    ...
    public setDebug(debug:Boolean) {
        if(debug && !this._debugNode) {
            ...
            this._debugNode.node.setPosition(new cc.Vec3(0, 0));
            ...
            if(loop) {
                for(let i = this._proxys.length - 1; i >= 0 ; i--) {
                    ...
                    if(!target.horizontalLoop) {
                        ...
                        copyed.region.x += this.scrollView.content._uiProps.uiTransformComp.width;
                        this._proxys.push(copyed);
                    }
                }
                this.scrollView.content._uiProps.uiTransformComp.setContentSize(this.scrollView.content._uiProps.uiTransformComp.width * 2, this.scrollView.content._uiProps.uiTransformComp.height);
            } else {
                for(let i = this._proxys.length - 1; i >= 0; i--) {
                    let target = this._proxys[i];
                    if(target.horizontalLoop) {
    ...
    private _checkLoopHorizontal() {
        ...
        if(Math.abs(delta) > 2) {
            ...
            let originalWidth = this.scrollView.content._uiProps.uiTransformComp.width / 2;
    ...
    private _refreshViewportRect() {
        ...
        this._rectCachedViewport.size = this._svCachedScrollView.node._uiProps.uiTransformComp.contentSize;
        this._rectCachedViewport.origin = cc.v2(-this._svCachedScrollView.content._uiProps.uiTransformComp.anchorX * this._svCachedScrollView.content._uiProps.uiTransformComp.width, 
            (1 - this._svCachedScrollView.content._uiProps.uiTransformComp.anchorY) * this._svCachedScrollView.content._uiProps.uiTransformComp.height - this._rectCachedViewport.size.height);
        this._checkUIState();
        this._syncingUI = true;
    }
