    ...
    async start() {
        ...
        await this.walkDirSync(FormsPath, async (prefabUrl: string, stat: any) => {
            ...
            if(controlName){
                ...
                importContent += `import {${controlName}} from "./${Const.ControlImportFolder}/${controlName}";\n`;
            }
            return null;
        });
        let contentStr = ``;
...
`;
    ...
    getPrefabType(controlName: string, scriptsDir: string): string {
        if(!fs.existsSync){
            console.error("no sync function");
            return "";
        }
        if(!fs.existsSync(`${scriptsDir}/${Const.ControlImportFolder}/${controlName}.ts`)){
            console.warn(`${scriptsDir}/${Const.ControlImportFolder}/${controlName}.ts not exist`);
            return "";
        }
        let datastr = fs.readFileSync(`${scriptsDir}/${Const.ControlImportFolder}/${controlName}.ts`);
